...
# Импортируем 'db' из корневого пакета 'app'
from app import db
# Импортируем все модели из пакета 'app.models'
from app.models import (User, Role, Part, Stage, RouteTemplate, 
...
def seed_cypress_command():
    ...
    admin = User(username='admin', role=admin_role, full_name='Admin User')
    admin.set_password('password123')
    
    manager = User(username='manager', role=manager_role, full_name='Manager User')
    manager.set_password('password123')

    operator = User(username='operator', role=operator_role, full_name='Operator User')

...
from app import db
from app.models import Part, RouteTemplate, Permission, StatusHistory, AuditLog
from app.admin.part_forms import PartForm, EditPartForm, AddChildPartForm
from app.admin.action_forms import ChangeRouteForm, ConfirmForm, ChangeResponsibleForm
...
def add_single_part():
    ...
    if form.validate_on_submit():
        try:
            pcs.create_single_part(form, current_user, current_app.config)
...
def delete_part(part_id):
    ...
    try:
        pms.delete_single_part(part_to_delete, current_user, current_app.config)
...
def generate_single_qr(part_id):
    ...
    if form.validate_on_submit():
        ...
        if qr_img_bytes:
            db.session.add(AuditLog(part_id=part_id, user_id=current_user.id, action="Генерация QR", details=f"Создан QR-код для детали '{part_id}'.", category='part'))
            db.session.commit()
...
def qr_print_preview():
    ...
    parts_for_print = pus.get_parts_for_printing(part_ids)
...
def change_part_route(part_id):
    ...
    if StatusHistory.query.filter_by(part_id=part.part_id).first():
        flash('Нельзя изменить маршрут для детали, которая уже находится в производстве (имеет историю).', 'warning')
        return redirect(url_for('main.main_pages.history', part_id=part.part_id))

    form = ChangeRouteForm(obj=part)
    if form.validate_on_submit():
        was_changed = pms.change_part_route(part, form.new_route.data, current_user)
        if was_changed:
            flash(f"Маршрут для детали {part.part_id} успешно изменен.", 'success')
        else:
            # --- НАЧАЛО ИСПРАВЛЕНИЯ: Улучшаем сообщение ---
            flash("Вы выбрали тот же самый маршрут. Изменений не было.", "info")
            # --- КОНЕЦ ИСПРАВЛЕНИЯ ---
        return redirect(url_for('main.main_pages.history', part_id=part.part_id))
...
def cancel_stage(history_id):
    ...
    try:
        part, stage_name = pss.cancel_stage_by_history_id(history_id, current_user)
        ...
        return redirect(request.referrer or url_for('main.main_pages.dashboard'))
...
def bulk_action():
    ...
    if action == 'delete' and current_user.can(Permission.DELETE_PARTS):
        try:
            deleted_count = pms.delete_multiple_parts(part_ids, current_user, current_app.config)
...
def change_responsible(part_id):
    ...
    if form.validate_on_submit():
        was_changed = pms.change_responsible_user(part, form.responsible.data, current_user)
        if was_changed:
            flash('Ответственный за деталь успешно изменен.', 'success')
        else:
            # --- НАЧАЛО ИСПРАВЛЕНИЯ: Улучшаем сообщение ---
            flash("Вы выбрали того же ответственного. Изменений не было.", 'info')
            # --- КОНЕЦ ИСПРАВЛЕНИЯ ---
        return redirect(url_for('main.main_pages.history', part_id=part.part_id))
...
def add_child_part(parent_part_id):
    ...
    if form.validate_on_submit():
        try:
            pcs.create_child_part(form, parent_part_id, current_user)
    ...
    return redirect(url_for('main.main_pages.history', part_id=parent_part_id))